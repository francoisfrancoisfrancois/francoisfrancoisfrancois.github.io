...
function updateStickyTops() {
  document.querySelectorAll('.project').forEach(project => {
    const h = project.offsetHeight;
    const vh = window.innerHeight;
    project.style.top = (vh - h) + 'px';
  });
}
...
function updateFades() {
  ...
  projects.forEach((project, i) => {
    if (i === projects.length - 1) return; // pas de fade sur le dernier

    const next = projects[i + 1];
    const nextTop = next.getBoundingClientRect().top;
    const vh = window.innerHeight;

    // nextTop va de vh (pas encore arrivé) à 0 (complètement couvrant)
    const progress = 1 - (nextTop / vh);
    const clamped = Math.max(0, Math.min(1, progress));

    project.style.setProperty('--fade', clamped);
  });
}

window.addEventListener('scroll', updateFades, { passive: true });
document.addEventListener('DOMContentLoaded', updateFades);
